...
- **Sentence case** in UI text, headings, comments, prose, and commit messages. Code identifiers follow language conventions: camelCase (JS), UPPER_SNAKE_CASE (constants like `MENU_DOWNLOAD_SINGLE`), PascalCase (classes). The rule is about not writing English prose in Title Case, not about overriding language naming conventions.
